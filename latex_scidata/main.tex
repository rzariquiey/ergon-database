...
Figure~\ref{fig:feature_profile} shows the proportion of languages in ERGON coded as 1 (feature present) for each of the 24 features. Among the features with the highest prevalence rates are: \feat{GB409\_10} (indicative mood, 98\%), \feat{GB409\_20} (main clauses, 97\%), \feat{GB409\_8} (past tense, 97\%), \feat{GB409\_18} (animate A, 97\%), \feat{GB409\_12} (realis, 97\%), \feat{GB409\_7} (perfective, 96\%), \feat{GB409\_14} (agentive A, 96\%), and \feat{GB409\_5} (full NPs, 94\%). The lowest prevalence rates are found for \feat{GB409\_23} (ERG=GEN, 33\%), \feat{GB409\_1} (overt absolutive, 37\%), and \feat{GB409\_24} (ERG=INSTR/COM, 51\%). The low rate of overt absolutive marking is consistent with the well-established typological generalisation that the absolutive case tends to be zero-marked in ergative languages \cite{dixon1994,comrie1978}. The fact that the coded values reproduce this and other independently documented cross-linguistic patterns provides a first indication that the coding decisions in ERGON are sound.

\subsection{Internal consistency: implicational patterns}

The four person/animacy features (GB409\_2--5) offer a natural internal consistency check, since they jointly operationalise the implicational predictions of the Silverstein hierarchy \cite{silverstein1976}: ergative marking on a higher-animacy category (e.g., 2nd person pronouns) implies ergative marking on all lower-animacy categories (3rd person pronouns and full NPs). Because this implicational pattern is one of the best-documented tendencies in the typological literature \cite{comrie1989,bickel2008}, large-scale deviations from it in a database of ergative languages would more plausibly reflect coding errors than genuine typological signal.

Of 159 ergative languages with complete data on all four hierarchy features, 152 (95.6\%) conform to the implicational constraint. The predominant pattern is hier4 (ergative marking on all persons and NPs), found in 100 languages (62.9\% of those with complete data), followed by hier3 (NPs only, 23 languages, 14.5\%) and hier2 (3rd person and NPs, 16 languages, 10.1\%). Figure~\ref{fig:silverstein} presents the full distribution of configurations in an UpSet-style layout: bars report the frequency of each attested configuration, and the dot matrix below identifies which nominal categories carry ergative marking in each configuration; conforming configurations are shown in blue and violating configurations in red.

The seven languages that deviate from the implicational pattern (Table~\ref{tab:violations}) were individually re-verified against primary grammar sources during the curation process. In each case, the deviating configuration is explicitly supported by the descriptive source, and the supporting textual evidence is preserved in the Comment field of \texttt{values.csv}. These seven cases therefore constitute genuine typological exceptions rather than coding errors, consistent with the literature that treats the hierarchy as a strong statistical tendency rather than an absolute universal \cite{bickel2008}. In addition, as part of the curation protocol, all languages coded as (1,1,1,1) were individually reviewed by the first author against the source grammars, a process that identified and corrected several conflations of indexing and flagging (see Curation protocol).

\subsection{Consistency across paired features}

ERGON's paired features (perfective/imperfective, past/non-past, indicative/non-indicative, realis/irrealis) provide a second consistency check. Figure~\ref{fig:splits} summarises the distribution of value combinations across these four domain pairs: for the large majority of languages, the two members of each pair carry the same value, and the cases in which they diverge correspond to split-ergative systems explicitly described as such in the source grammars.

The phi coefficient between \feat{GB409\_8} (past tense) and \feat{GB409\_7} (perfective) is $\varphi = 0.910$ ($n = 156$). This near-perfect co-occurrence matches the theoretical expectation that past tense and perfective aspect pattern together in conditioning ergativity \cite{dixon1994,bybee1994}, and indicates that the two features were coded consistently across the database: systematic coding noise in either feature would attenuate this association.

\subsection{Feature associations}

Figure~\ref{fig:phi} presents pairwise phi coefficients across all 24 ERGON features, ordered and blocked by thematic domain. The association structure is coherent with the design of the feature set and with typological expectations: the tense/aspect/mood/reality status block (GB409\_6--13) shows consistently positive and often very high internal associations (past $\times$ perfective: $\varphi = .91$; indicative $\times$ past: $\varphi = .89$; indicative $\times$ perfective: $\varphi = .81$); the person features (GB409\_2--4) form a second coherent cluster (1st $\times$ 2nd person: $\varphi = .93$; 2nd $\times$ 3rd person: $\varphi = .67$; 1st $\times$ 3rd person: $\varphi = .60$), with weaker associations to \feat{GB409\_5} (full NPs), as expected under the implicational patterns discussed above; and the formal-overlap features (\feat{GB409\_23}, \feat{GB409\_24}) are largely independent of the distributional features, as expected given that they capture properties of the form of the marker rather than its distribution. The absence of anomalous associations (e.g., strong negative correlations within a thematic block) provides further evidence against systematic coding artefacts.

\subsection{External consistency with Grambank}

As an external validation, we compared ERGON's classification of languages as ergative with Grambank's coding of the parent feature GB409 \cite{grambank2023}. For the languages present in both datasets, the vast majority receive consistent classifications: languages coded as ergative in ERGON are coded as 1 (ergative alignment of flagging present) in Grambank. The discrepancies fall into three types: (1) marginal or split-ergative systems that fall on the boundary of the binary coding, where reasonable coders may disagree; (2) differences in the source grammars consulted; and (3) cases where the ERGON curation process identified the earlier coding as based on verbal indexing rather than case flagging (e.g., Bugis, Pitu Ulunna Salu). All such cases are documented in the Comment field of \texttt{values.csv}, so that users can trace the evidence behind each classification decision.
